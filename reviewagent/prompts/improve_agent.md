...
- **多行替换警告**: 若 improved_code 行数 > existing_code 行数，improved_code 末尾**必须保留** existing 之后的所有源码行。
  例如替换 docstring 时，`existing_code` 必须包含 docstring 全部行 + return 行，
  `improved_code` 也必须包含新 docstring + 原 return 行，不能只写新 docstring 丢掉 return。
- **收缩警告**: 若 improved_code 行数 < existing_code 行数（建议在删除代码），
  只能删除 diff 中标记为 `-`（已删除）的行，不能把 existing 中未变更的上下文行也删除。
  `existing_code` 只应该包含你实际要替换/修改的行，不要包含无关的上下文。
- 最多 8 条建议 — 少而准
