...
def test_overview_table_contains_status_summary_and_findings(tmp_telemetry):
    """_build_overview_table 应生成包含状态汇总 + 本次新发现 + 待处理明细的 markdown 表格.

    验证:
    - 顶部状态表有 ⏳ 待处理 / ✅ 已采纳 / ❌ 已忽略 三种状态
    - 本次新发现 (inline_posted) 列出 file/line/severity/header/rule
    - 待处理建议按 file+line 排序, 区分历史累积
    - 底部有时间戳 + V{N} 标识
    """
    from reviewagent.commands.improve import ImproveCommand
    from reviewagent.telemetry.store import get_store
    ...
    # 历史建议: 1 open (HIGH) + 1 applied + 1 dismissed
    ...
    # 模拟一次 run: 1 open + 1 dismissed
    s.record_run = lambda **kw: None  # 跳过 run 记录, 直接用 suggestion_stats
    # 用 s.list_runs 时不返回任何行, 所以 V=1
    # 准备本次 inline_posted (含 1 条新建议)
    cmd = ImproveCommand.__new__(ImproveCommand)
    cmd.project_id = 34
    cmd.mr_iid = 300
    ...
    inline_posted = [{
        "note_id": "new-1",
        "raw": {
            "file": "services/sample_new.py",
            "start_line": 5,
            "severity": "high",
            "header": "SQL 注入",
            "rule_keys": ["SSD-RULE-SQL-INJECTION"],
        },
        "normalised": {
            "file": "services/sample_new.py",
            "new_line": 5,
    ...
    inline_skipped = [
        {"suggestion": {"file": "x.py"}, "reason": "duplicate_at_line"},
    ]

    out = cmd._build_overview_table(inline_posted, inline_skipped, total_agent_suggestions=5)

    # === 验证 ===
    # 1. 顶部状态汇总表
    assert "## 检视总览 V" in out, f"应有 '## 检视总览 V{{N}}' 标题: {out!r}"
    assert "| 状态 | 数量 | 备注 |" in out
    assert "| 总建议 | 3 |" in out, f"应有 3 条累计: {out!r}"
    assert "| ⏳ 待处理 | 1 |" in out, f"应有 1 条待处理: {out!r}"
    assert "| ✅ 已采纳 | 1 |" in out, f"应有 1 条已采纳: {out!r}"
    assert "| ❌ 已忽略 | 1 |" in out, f"应有 1 条已忽略: {out!r}"
    # 2. 本次新发现明细
    assert "### 本次新发现" in out
    assert "SQL 注入" in out
    assert "`sample_new.py`" in out
    assert "🔴 HIGH" in out, f"应有 HIGH 严重度 emoji: {out!r}"
    assert "SSD-RULE-SQL-INJECTION" in out
    # 3. 本次未发布
    assert "### 本次未发布" in out
    assert "重复" in out or "校验失败" in out
    # 4. 待处理建议 (历史 open)
    assert "### 待处理建议" in out
    assert "可变默认参数" in out, f"历史 open 应在待处理中: {out!r}"
    # 5. 底部时间戳 + V{N}
    assert "最后更新:" in out
    assert "UTC" in out


def test_overview_table_works_with_empty_state(tmp_telemetry):
    """MR 第一次检视前, telemetry 为空, 表格应仍可生成 (不抛异常).

    验证 _build_overview_table 在 telemetry query 失败或 store 为空时
    不 crash, 至少生成空状态汇总.
    """
    from reviewagent.commands.improve import ImproveCommand
    from reviewagent.telemetry.store import get_store
    ...
    cmd.project_id = 34
    cmd.mr_iid = 999  # 没数据
    out = cmd._build_overview_table([], [], total_agent_suggestions=0)

    assert "## 检视总览 V" in out
    assert "| 总建议 | 0 |" in out
    assert "| ⏳ 待处理 | 0 |" in out
    assert "| ✅ 已采纳 | 0 |" in out
    assert "| ❌ 已忽略 | 0 |" in out
    # 没 inline_posted 时不应有 "本次新发现" 段
    assert "### 本次新发现" not in out
